...
(
    ...
    const List<scalar>& pvWeights,
    scalar& LeZ,
    scalar& LeC
) const
{
    if (pvWeights.size() && pvWeights.size() != pvIds.size())
    {
        FatalErrorInFunction
            << "pvWeights (" << pvWeights.size() << ") must be empty or match "
            << "pvIds (" << pvIds.size() << ")" << exit(FatalError);
    }
    ...
    // PV-weighted mixture-averaged diffusivity for the progress variable.
    // The weight is w_k*Y_k with the SAME w_k that define the tabulated C, so
    // D_C is the diffusivity of the scalar the solver actually transports.
    scalar wsum = 0, DC = 0;
    forAll(pvIds, k)
    {
        const label i = pvIds[k];
        const scalar w =
            (pvWeights.size() ? pvWeights[k] : scalar(1))*max(Yl[i], scalar(0));
        if (w > 0)
        {
            wsum += w;
...
template<class ThermoType>
bool Foam::SRKchungTakaMixture<ThermoType>::transportProps
(
    const List<scalar>& Y,
    const scalar p,
    const scalar T,
...
) const
{
    ...
    // compute for this composition.
    mu = mixture_.mu(p, T);
    kappa = mixture_.kappa(p, T);
    Cp = mixture_.Cp(p, T);
    rho = mixture_.rho(p, T);
    ...
    Dmix.setSize(nSpecies);
    for (label i = 0; i < nSpecies; i++)
    {
        Dmix[i] = mixture_.Dimix(i, p, T);
    }
